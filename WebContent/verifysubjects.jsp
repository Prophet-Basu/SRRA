...
<%@page import="com.bean.TeachesBean"%>
...
a{
 ...
 <body>
  ...
  <center>
	...
	TeacherDao t2dao=new TeacherDaoImpl();
	ArrayList<TeachesBean> tlist=tdao.fetchNonVerifiedSubjects(strID);
	ArrayList<String> tnamelist=new ArrayList<>();
	TeacherBean tbean;
	for(TeachesBean tb:tlist){
		tbean=t2dao.fetchTeacher(tb.getTeacherID(), strID);
		tnamelist.add(tbean.getTeacherName());
	}
	if(tlist.size()>0)
	{
	...
	<table>
	<tr>
	<th>Teacher Name</th> <th>Session</th> <th>Section Name</th> <th>Subject Code</th><th>Click to Verify</th>
	</tr>
	<%
		for(TeachesBean tb:tlist){
			tbean=t2dao.fetchTeacher(tb.getTeacherID(), strID);
	%>
	<tr>
		<td><%=tbean.getTeacherName() %></td>
		<td><%=tb.getSesID() %></td>
		<td><%=tb.getSesSecName() %></td>
		<td><%=tb.getSubCode() %></td>
		<td><a href="TeacherSubjectVerifySrv?sesID=<%=tb.getSesID() %>&secName=<%=tb.getSesSecName()%>">Verify</a></td>
	</tr>
	<%	
		}
	%>
	...
	<%
		...
		session.setAttribute("MSG", "No subjects to display");
